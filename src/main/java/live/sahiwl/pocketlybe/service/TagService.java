...
public class TagService {
    ...
    public TagResponseDTO createTag(TagRequestDTO req){
        Tag tag = tagRepository.findByTitle(req.getTitle())
                .orElseGet(() -> tagRepository.save(Tag.builder().title( req.getTitle()).build()));
        return new TagResponseDTO(tag.getId(), tag.getTitle());
    }

    public List<TagResponseDTO> getAllTags(){
        return tagRepository.findAll().stream().map(tag-> TagResponseDTO.builder().id(tag.getId()).title(tag.getTitle()).build()).collect(Collectors.toList());
    }
}
